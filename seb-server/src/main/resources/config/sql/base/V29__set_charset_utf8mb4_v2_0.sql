...
-- -----------------------------------------------------
-- Temporarily drop the FK so MariaDB can alter both referenced columns.
ALTER TABLE `SEBServer`.`client_instruction`
  DROP FOREIGN KEY `instructionConnectionRef`;
...
ALTER TABLE `SEBServer`.`client_instruction`
  ADD CONSTRAINT `instructionConnectionRef`
